...
/*  @Override
  ...
  public List<ActorVO> list(HashMap<String, Object> hashMap) {
    ...
    while(iter.hasNext() == true) {
      ...
      actorname = Tool.convertChar(actorname); // 태그 처리
  ...
  public String paging(int search_count, int nowPage, String s_word){ 
    int totalPage = (int)(Math.ceil((double)search_count/Actor.RECORD_PER_PAGE)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/Actor.PAGE_PER_BLOCK));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/Actor.PAGE_PER_BLOCK));    // 현재 그룹 
    int startPage = ((nowGrp - 1) * Actor.PAGE_PER_BLOCK) + 1; // 특정 그룹의 페이지 목록 시작  
    int endPage = (nowGrp * Actor.PAGE_PER_BLOCK);             // 특정 그룹의 페이지 목록 종료   
